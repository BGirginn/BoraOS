#!/usr/bin/env bash
# BoraOS x86_64 Master Build Script
# Executes all build steps in order to create reproducible x86_64 ISO
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║${NC}  ${GREEN}BoraOS x86_64 0.1 - Reproducible Build Pipeline${NC}   ${BLUE}║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "Run with: sudo ./build-x86.sh"
    exit 1
fi
...
ARCH=$(uname -m)
if [ "$ARCH" != "x86_64" ]; then
    echo -e "${YELLOW}WARNING: Not running on x86_64 architecture${NC}"
    echo "Current architecture: $ARCH"
    echo "x86_64 build should be performed on x86_64 system"
    echo ""
...
# Change to BoraOS x86_64 directory
cd "$BORAOS_DIR"

# Build steps
STEPS=(
    "prepare_environment_x86.sh:Prepare x86_64 Build Environment"
    "clean_previous_build.sh:Clean Previous Build"
    "sync_profile_x86.sh:Sync x86_64 Profile Files"
    "build_bootloader_x86.sh:Configure x86_64 Bootloader"
    "build_image_x86.sh:Build x86_64 ISO"
    "finalize_x86.sh:Finalize x86_64 Output"
...
echo "2. Test in VM: Use UTM, QEMU, or VirtualBox"
echo "3. Write to USB: dd if=boraos-0.1-x86_64.iso of=/dev/sdX bs=4M"
echo ""
